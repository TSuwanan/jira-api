...
const isConnectionError = (error: any): boolean => {
  ...
  return (
    connectionErrors.some(msg => errorMessage.includes(msg)) ||
    ['ECONNRESET', 'ECONNREFUSED', 'ENOTFOUND', 'ETIMEDOUT', '57P01', '08003', '08006'].includes(
      errorCode,
    )
  )
}
...
async function retryQuery<T>(queryFn: () => Promise<T>, retries = MAX_RETRIES): Promise<T> {
  let lastError: any
...
export async function testConnectionWithRetry(): Promise<void> {
  try {
